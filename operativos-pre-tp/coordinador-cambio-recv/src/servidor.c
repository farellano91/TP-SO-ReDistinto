	...
	case 3:
		...
		if(controlo_existencia(instancia_nueva)){
			...
			while(1){
				...
				int32_t numbytes = 0;
				//1:falle 2:ok
				if ((numbytes = recv(fdCliente, &respuestaInstacia, sizeof(int32_t), 0)) <= 0) {
					if (numbytes == 0) {
					// conexión cerrada
						printf("Se fue el INSTANCIA FD: %d\n",fdCliente);

					} else {
						perror("ERROR: al recibir respuesta de la INSTANCIA");

					}
					//Saco a la instancia de la lista(la busco por el fd)
				}
				VG_RESPUESTA_INSTANCIA = respuestaInstacia;
				if(VG_RESPUESTA_INSTANCIA != 0){
					printf("Recibimos respuesta de Instancia\n");

				}
				pthread_cond_signal(&COND_RECV_INSTANCIA);
...
void levantar_servidor_coordinador() {
	...
	pthread_cond_destroy(&COND_RECV_INSTANCIA);
}
